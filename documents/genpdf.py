...
def obj_pdf(obj_id, obj_record):
    ...
    c.drawCentredString(width / 2, height - 150, process_arabic_text("ديوان وزارة الاقتصاد والتجارة"))
    c.setFont("Amiri-bold", 14)
    c.drawCentredString(width / 2, height - 175, process_arabic_text("اشهار مبدئي لعلامة تجارية"))

    c.line(30, height - 195, width - 30, height - 195)

    # Date and trans_id
    c.setFont("Amiri", 14)
    c.drawCentredString(width / 2, height - 225, process_arabic_text(f"طلب مقدم لتسجيل علامة تجارية رقم: ({obj_record['obj_number']})"))
